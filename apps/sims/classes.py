...
# TIMEOUT para todas as conexões HTTP (evita tasks travadas)
    ...
    def listPlan(operator):
        if operator == 'CMHK':
            planList = {
                ...
                'chip-internacional-america-do-norte',
                'chip-internacional-africa',
                'chip-internacional-oriente-medio',
...
class selectPlanCMHK():
    ...
    def selectPlanCod(plan, day, data):
        list_name = selectPlanCMHK.selectPlanList(plan)
        if not list_name:
            return None
